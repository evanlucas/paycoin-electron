...
Wallet.prototype.render = function(info, accounts) {
  ...
  return [
    h('#receive.bg-light', [
      h('.wrapper', [
        h('.tab-text', [
          h('h3.text-primary.col-sm-6.m-none.no-padder.m-b-md', 'Receive Paycoins')
        , h('a.btn.btn-dark.pull-right.col-sm-3.col-sm-offset-3', { "href": "#generateAddress"}, [
            h('font-thin.m-t-b-a]', [
              "Generate Address"
            ])
          ])
...
function address(item) {
  ...
  , h('td', { "style": "height:40px;" }, balance)
  ])
}
